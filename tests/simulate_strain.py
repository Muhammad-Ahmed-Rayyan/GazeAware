...
def simulate_overlay():
    """
    MODE 9: Ghost Overlay System Test
    ──────────────────────────────────────────────────────────────────────────
    Starts the Vitality Ring HUD and ramps the strain score from 0 → 95
    over 30 seconds to trigger the Forced Recovery animation automatically.

    Watch the corner of your screen for the semi-transparent ring, then
    observe the full-screen ball-pursuit overlay fire when score hits 90.
    """
    print("\n" + SEP60)
    print("  MODE 9: Ghost Overlay System Test")
    print(SEP60)
    print("  Starting Vitality Ring HUD in bottom-right corner (10% opacity).")
    print("  Score will ramp 0 → 95 over 30s to trigger Forced Recovery at 90.")
    print("  Hover the ring corner to see it at full opacity.")
    print("  During recovery animation, press Esc twice to dismiss early.")
    print(SEP60 + "\n")

    overlays = OverlayManager()
    overlays.start()
    print("  [Overlay] Started — check bottom-right corner of screen.")
    time.sleep(1.0)   # let tkinter window appear

    engine  = StrainFusionEngine()
    ramp_duration = 30.0   # seconds to go from 0 → 95
    start = time.time()

    try:
        while True:
            elapsed = time.time() - start
            ramp_score = min(95.0, (elapsed / ramp_duration) * 95.0)

            # Build synthetic signal profile proportional to ramp score
            t = ramp_score / 95.0
            signals = {
                k: min(1.0, v * t)
                for k, v in PROFILES["critical"].items()
            }

            score, zone, _label = engine.compute_and_print(
                signals,
                extra=f"[SIM-OVERLAY] ramp={ramp_score:.0f}",
            )
            overlays.update(score=score, zone=zone)

            if ramp_score >= 95.0 and not overlays.forced_active:
                # Keep running for a while after recovery completes
                time.sleep(30.0)
                break
